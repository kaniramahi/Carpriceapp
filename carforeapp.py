...
with open('dt_modelcar..pkl', 'rb') as f:
    loaded_pipeline = pickle.load(f)
 
# Load the data
df = pd.read_pickle('datacar..pkl')

# Create the Streamlit app
st.title('🚗 Used Car Selling Price Prediction App by Krishna')
...
# Make the prediction
prediction = loaded_pipeline.predict(model)[0]
 
 
# Display the predicted selling price
st.sidebar('Predicted Selling Price')
st.write('The predicted selling price of the used car is'  'units.')
